...
**Your Answers here:**
<br> Answer 1: This is done in the beginning when we state mytickers = c("SPY", "AAPL"), which tells R to download data relating to both S&P 500 and Apple together. File used is DataSet1.Rdata.
<br>Answer 2: 
1. The cumulative returns of the Apple index during this period is `r round(100*sum(StockReturns[,"AAPL"]),1)`%.
2. The average daily returns of the Apple index during this period is `r round(100*mean(StockReturns[,"AAPL"]),3)`%;
3. The standard deviation of the daily returns of the Apple index during this period is `r round(100*sd(StockReturns[,"AAPL"]),3)`%;
<br>
<br>
