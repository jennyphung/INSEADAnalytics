...
*Answer 1: We need atleast 6 components to explain atleast 50% of the variance. 
*Answer 2: The highest weights of the first principal component portfolio on the 64 futures is 5 yr T-Notes US and 10 yr T-Notes.  
*Answer 3: Yes. The first 10 components can be considered as follows:
1. Western securities
2. Different currencies and major commodities
3. Stock exchanges excluding North American stock exchanges
4. North American stock exchanges
5. European securities
6. Oil & Gas
7. Raw materials & commodities
8. Agricultural commodities
9. Precious metals
10. Asian stock exchanges
Answer 4: 


```{r echo=TRUE, results='markup'}
crisis_data = futures_data[as.Date(rownames(futures_data))>"2007-10-01"&as.Date(rownames(futures_data))<"2009-03-01",]
print(colnames(crisis_data))
```
...
Variance_Explained_Table2<-Variance_Explained_Table_results2$eig
colnames(Variance_Explained_Table2)<-c("Eigenvalue", 
  "Pct of explained variance", "Cumulative pct of explained variance")
```

```{r}
show_data = round(Variance_Explained_Table2, 2)
iprint.df(show_data)
```

```{r echo=TRUE}
eigenvalues2  <- Variance_Explained_Table2[, "Eigenvalue"]
```
...
Let's now see how the 20 first principal components look like.
...
Future returns during bear markets are more highly correlated than during bull markets. Thus, analysis for bull markets will require fewer components. 
...
*Answer 1: The code takes average future prices for different durations, in days, and scales them to $1 being invested every day. The plot is return on year/month.
*Answer 2: MAfreq<-seq(8,240,by=20)
*
*
* 
*
